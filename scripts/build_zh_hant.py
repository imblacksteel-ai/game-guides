...
#   超級機器人大戰DD: 公式臺港澳版 https://srw-dd-tw.suparobo.jp/
OFFICIAL_TITLES = {
...
# スパロボDDは臺港澳向けの公式繁体中文版がある（srw-dd-tw.suparobo.jp、2019.08.21配信）ため、
...
OVERRIDES = {
    "games/srwdd/index.html": [
        ("沒有官方中文/英文版——本頁用簡明的語言整理了抽卡、前期攻略和連鎖系統。",
         "臺灣、香港、澳門有官方繁體中文版——本頁整理了抽卡、前期攻略和連鎖系統。"),
        ...
        ("與系列大多數主機作品不同，本作至今沒有官方在地化版本，即便系列其他作品已有。這個頁面是為已經在玩日文版、但想要一份清晰參考資料的玩家準備的。",
         "除日文版外，臺灣、香港、澳門另有官方繁體中文版。本頁內容以日文版為準整理，繁體中文版遊戲內的部分名稱譯法可能與本頁不同。"),
        ("所涉遊戲沒有官方英文/中文在地化版本。",
         "內容以日文版為準整理，繁體中文版遊戲內的名稱譯法可能與本頁不同。"),
    ],
    "index.html": [
        ("抽卡目標、新手上路指南、連鎖系統傷害加成表與核心術語——沒有官方在地化的高達/EVA聯動RPG。",
         "抽卡目標、新手上路指南、連鎖系統傷害加成表與核心術語——高達/EVA聯動戰術RPG（臺港澳有官方繁中版）。"),
    ],
}
...
def convert_text(text):
    ...
    def hold(m):
        run = m.group(0)
        # カナに隣接する漢字も日本語として保護するが、そこに変換対象の漢字があるなら
        # 中国語の文が日本語に隣接して巻き込まれている可能性が高い（例:「进行ケッコン…」）。
        # 黙って簡体字を残さないよう止める。原文で「」や括弧を挟んで区切ること。
        if m.re is JP_RUN:
            kanji = "".join(re.findall(r"[\u4e00-\u9fff]", run))
            if kanji and converter.convert(kanji) != kanji:
                raise SystemExit(f"Chinese text runs into a Japanese term, separate it in the source: {run}")
        kept.append(run)
        return f"\ue000{len(kept) - 1}\ue001"
